...
def test_expressive_motion_preserves_authored_arrival_and_hold_times(
    motion_path, expected_times, project_poses, project_limits
):
    requested = load_requested(motion_path, project_poses, project_limits)
    start = pose_positions(project_poses, "attentive", requested.joint_names)
    generated = generate_trajectory(
        requested, start, (0.0,) * len(start), project_limits
    )

    assert [point.time_from_start for point in generated.points] == pytest.approx(
        expected_times
    )
