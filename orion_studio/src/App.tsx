...
export default function App() {
  ...
  return (
    <main className={`studio-shell ${destination === "home" ? "home-shell" : "create-shell"}`} id="main-content">
      <a className="skip-link" href="#workspace">Skip to workspace</a>
      <header className="topbar">
        <button className="brand-lockup" onClick={() => { setDestination("home"); setPlaying(false); }} aria-label="Open Orion Studio home"><span className="brand-mark"><i /><i /><i /><i /><i /><i /><i /><i /></span><span><strong>ORION</strong><small>CHARACTER STUDIO</small></span></button>
      ...
      {destination === "home" && <Home voiceLabel={voiceLabel} connection={connection} status={status} onConnect={() => setConnectionOpen(true)} onVoice={() => setVoiceOpen(true)} onCreate={() => setDestination("create")} onRefresh={() => pairing.refresh()} onNotice={setNotice} onRun={setTrackedRun} />}
      {destination === "create" && <>
