...
def test_report_contains_sources_generated_path_and_measured_start():
    ...
    assert report["execution"]["backend"] == "gazebo"
    assert report["environment"]["ros_packages"]["controller_manager"]
    assert report["environment"]["ros_packages"]["gz_ros2_control"]


def test_matching_backends_pass_shared_contract_comparison():
    gazebo = make_report("gazebo")
    mujoco = make_report("mujoco_ros2_control")

    comparison = compare_run_reports(gazebo, mujoco)
...
def test_source_or_outcome_mismatch_fails_comparison():
    gazebo = make_report("gazebo")
    mujoco = deepcopy(make_report("mujoco_ros2_control"))
