...
class RoutinesDaemonTests(DaemonTestCase):
    def test_lamp_mode_stays_awake_and_alert_rings_at_rest_without_enabling_torque(self):
        with daemon(automatic_character=True, rest_after_seconds=1, following=True) as path:
            ...
            self.ok(path, 'routines {"action":"set_sound","kind":"timer","sound":"funny_alarm"}')
            self.ok(path, 'routines {"action":"timer","seconds":1,"label":"tea"}')
            ringing = self.wait_for(path, 'routines status', lambda value: value['routines']['ringing'])
            self.assertEqual(ringing['routines']['ring_sound'], 'funny_alarm')
            self.ok(path, 'routines {"action":"set_sound","kind":"timer","sound":"club_alarm"}')
            self.assertEqual(request(path, 'routines status')['routines']['ring_sound'], 'funny_alarm')
